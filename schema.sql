CREATE DATABASE IF NOT EXISTS LifeThread;
USE LifeThread;
...
BEGIN
  SELECT AppointmentID, Patient.Name, Physician.Name, Time FROM User AS Patient, User AS Physician, Appointment WHERE Patient.UserID = PatientID AND Physician.UserID = PhysicianID AND Patient.UserID = id;
END//
...
BEGIN
  SELECT * FROM MedicalRecord NATURAL JOIN Prescription WHERE PatientID = patient_id AND RxNumber IS NOT NULL ORDER BY Timestamp;
END//
